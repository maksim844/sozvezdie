...
class easyphpthumbnail {
	 ...
	 * @param string $output
	 */	
	public function Createthumb($filename="unknown",$output="file") {
	...
	private function rectangle() {
		...
		$squaresize=max($this->thumbx,$this->thumby);
		$this->newimage=imagecreatetruecolor($this->RectW ,$this->RectH);
		imagefilledrectangle($this->newimage,0,0,$this->RectW,$this->RectH,imagecolorallocate($this->newimage,hexdec(substr($this->Backgroundcolor,1,2)),hexdec(substr($this->Backgroundcolor,3,2)),hexdec(substr($this->Backgroundcolor,5,2))));
		$centerx=floor(($this->RectW-$this->thumbx)/2);
		$centery=floor(($this->RectH-$this->thumby)/2);
		imagecopy($this->newimage,$this->thumb,$centerx,$centery,0,0,$this->thumbx,$this->thumby);
		imagedestroy($this->thumb);
		$this->thumb=imagecreatetruecolor($this->RectW,$this->RectH);
		imagecopy($this->thumb,$this->newimage,0,0,0,0,$this->RectW,$this->RectH);
		imagedestroy($this->newimage);
